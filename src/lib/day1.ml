...
let make_int_from_acc acc =
  try
    Int.of_string @@ String.of_char_list [ List.hd_exn acc; List.last_exn acc ]
  with _ -> 0
...
let parse_written_number x =
  let%bind _, n =
    List.find ~f:(fun (wn, _) -> String.is_prefix ~prefix:wn x) written_numbers
  in
  Some n

let%test "Parsing written number (1)" =
  Option.equal Int.equal (parse_written_number "nine") @@ Some 9

let%test "Parsing written number (2)" =
  Option.equal Int.equal (parse_written_number "eight0z") @@ Some 8

let%test "Parsing written number (3)" =
  Option.equal Int.equal (parse_written_number "tone0") @@ None

let take_head_off s =
  try String.sub ~pos:1 ~len:(String.length s - 1) s with _ -> s
